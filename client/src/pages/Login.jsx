...
import { closeModal, openModal } from "../service/ui-slice";
...
const Login = () => {
  ...
  const onSubmit = async (data) => {
    try {
      ...
      dispatch(openModal({ modalId: "loginError" }));
    }
  };
    ...
    <>
      ...
      <Modal id="loginError">
        <div className="flex flex-col gap-3 w-full items-center">
          <p>{error?.data?.message || "An error occurred during login"}</p>
          <Button
            className="primeBtn w-[10rem]"
            onClick={() => dispatch(closeModal({ modalId: "loginError" }))}
          >
            Ok
          </Button>
        </div>
